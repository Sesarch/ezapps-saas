...
export default function StoresPage() {
  ...
  const connectShopify = () => {
    ...
    // ONLY send the store name - nothing else
    // Example: user types "ezapps-2" or "ezapps-2.myshopify.com"
    // We extract just "ezapps-2" and send that
    ...
    // Send ONLY the store name to the API
    window.location.href = `/api/auth/shopify?shop=${encodeURIComponent(storeName)}`
  }
